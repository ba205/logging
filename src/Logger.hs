...
data Logger = Logger { masterLog       :: PSQ Text Int
                     , queueBound      :: Int
                     , defaultPriority :: Int
                     }
                     deriving (Show)
...
createLogger :: Logger
createLogger = 
  Logger { masterLog = P.empty, queueBound = 1024, defaultPriority = 3 }
...
updateLogger priority message (Logger {masterLog, queueBound, defaultPriority})
  = case P.size masterLog >= queueBound of
      True  -> Left "Can't add message; queue is full"
      False -> let newLog = P.insert message priority masterLog
